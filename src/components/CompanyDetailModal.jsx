...
const TIMELINE_EVENTS = c => [
  ...
  { key: 'scored',  label: 'AI skóre vypočítané', done: c.aiScore != null,  time: null },
      ...
      <div style={css.modal}>
        ...
        <div style={css.header}>
          <div style={css.headerLeft}>
            ...
            <div style={css.headerMeta}>
              ...
              {company.city && <span style={css.dot}>·</span>}
              {company.city && <span>{company.city}</span>}
              {company.country && <span style={css.dot}>·</span>}
        ...
        <div style={css.cols}>
          ...
          <div style={css.col}>
            ...
            <InfoRow label="Web">
              {company.website
                ? <a href={`https://${company.website}`} target="_blank" rel="noreferrer" style={css.link}>{company.website}</a>
                : <Muted>–</Muted>}
            </InfoRow>
            ...
            <InfoRow label="Place ID"> <Muted style={{ fontSize: '0.58rem' }}>{company.googlePlaceId || '–'}</Muted></InfoRow>
          </div>
          ...
          <div style={css.col}>
            ...
            {pri && (
              <div style={{ ...css.priBadge, color: pri.color, borderColor: pri.color + '44' }}>
                {pri.label} potenciál
              </div>
            )}
            {positive.length > 0 && (
            ...
            {risks.length > 0 && (
              <>
                <div style={{ ...css.factorTitle, marginTop: '0.65rem' }}>Riziká</div>
                {risks.map((r, i) => <div key={i} style={css.factorRisk}>⚠ {r}</div>)}
              </>
            )}
            {nextStep && (
              <div style={css.nextStep}>
                <div style={css.factorTitle}>Odporúčaný krok</div>
                <div style={css.nextStepText}>→ {nextStep}</div>
              </div>
            )}
          </div>
          ...
          <div style={css.col}>
            ...
            <InfoRow label="Priorita">
              {pri
                ? <span style={{ color: pri.color, fontFamily: mono, fontSize: '0.68rem' }}>{pri.label}</span>
                : <Muted>–</Muted>}
            </InfoRow>
            ...
            <div style={{ marginTop: '1rem' }}>
              ...
              <div style={css.taskRow}>
                <input style={css.taskInput} placeholder="Popis úlohy..."
                  ...
                  onKeyDown={e => { if (e.key === 'Enter' && taskText.trim()) { onCreateTask && onCreateTask(company.id, taskText.trim()); setTask('') } }} />
        ...
        <div style={css.section}>
          ...
          <div style={css.emailBtns}>
            <button style={css.btnPrimary} onClick={() => { onGenerateEmail && onGenerateEmail(company); setDOpen(true) }}>
              ✦ Generovať draft
            </button>
          ...
          {draftOpen && (
            <div style={css.draftBox}>
              ...
              <input style={css.draftInput} value={draftSubj} onChange={e => setDSubj(e.target.value)} placeholder="Predmet emailu..." />
              <label style={css.draftLabel}>Text</label>
              <textarea style={css.draftArea} value={draftBody} onChange={e => setDBody(e.target.value)} placeholder="Text emailu..." />
            </div>
          )}
        </div>
          ...
          <div style={css.timeline}>
            {timeline.map((ev, i) => (
              <div key={ev.key} style={css.tlItem}>
                ...
                {i < timeline.length - 1 && <div style={{ ...css.tlLine, background: ev.done ? '#00cc8844' : '#1e2530' }} />}
                <div style={css.tlLabel}>
                  <span style={{ color: ev.done ? '#e8eaed' : '#4b5563' }}>{ev.label}</span>
...
function ColTitle({ children }) {
  return <div style={{ fontFamily: mono, fontSize: '0.55rem', letterSpacing: '3px', textTransform: 'uppercase', color: '#4b5563', marginBottom: '0.75rem', paddingBottom: '0.35rem', borderBottom: '1px solid #1e2530' }}>{children}</div>
}
function InfoRow({ label, children }) {
  return (
    <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.35rem', alignItems: 'baseline' }}>
...
const css = {
  overlay:    { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 500, overflowY: 'auto', padding: '2rem 1rem', display: 'flex', justifyContent: 'center', alignItems: 'flex-start' },
  modal:      { background: '#0d1117', border: '1px solid #21262d', borderRadius: 6, width: '100%', maxWidth: 1080, padding: '2rem', position: 'relative' },
  ...
  cols:       { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' },
  col:        { minWidth: 0 },

  aiReason:   { fontFamily: mono, fontSize: '0.65rem', color: '#9ca3af', fontStyle: 'italic', marginBottom: '0.75rem', lineHeight: 1.6, background: 'rgba(255,255,255,0.03)', padding: '0.5rem 0.6rem', borderRadius: 3 },
  priBadge:   { fontFamily: mono, fontSize: '0.6rem', letterSpacing: '1px', textTransform: 'uppercase', padding: '0.2rem 0.5rem', borderRadius: 2, border: '1px solid', display: 'inline-block', marginBottom: '0.75rem' },
  factorTitle:{ fontFamily: mono, fontSize: '0.52rem', letterSpacing: '2px', textTransform: 'uppercase', color: '#4b5563', marginBottom: '0.3rem' },
  factorPos:  { fontFamily: mono, fontSize: '0.65rem', color: '#00cc88', lineHeight: 1.8 },
  factorRisk: { fontFamily: mono, fontSize: '0.65rem', color: '#ffaa00', lineHeight: 1.8 },
  nextStep:   { marginTop: '0.65rem' },
  nextStepText:{ fontFamily: mono, fontSize: '0.65rem', color: '#00cc88', lineHeight: 1.6 },
  ...
  divider:    { height: 1, background: '#161b22', margin: '1.25rem 0' },
