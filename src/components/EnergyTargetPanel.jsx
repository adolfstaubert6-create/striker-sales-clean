...
export default function EnergyTargetPanel({ view = 'dashboard', setView }) {
  ...
  const FILTERS = [{ key: 'all', label: 'Všetky' }, ...INTEL_STATUSES]

  return (
    <div style={{ padding: '1.25rem', maxWidth: 1100, margin: '0 auto', width: '100%' }}>

      {/* 1. IntelSummaryPanel — rovnaké miesto a pocit ako AiSummaryPanel */}
      <IntelSummaryPanel targets={targets} />

      {/* 2. IntelAgentPanel — rovnaké miesto a pocit ako AgentPanel */}
      <IntelAgentPanel
        onDone={() => {}}
        onAdded={newTarget => setSelected(newTarget)}
      />

      {/* 3. Toolbar — rovnaká štruktúra ako Dashboard toolbar */}
      ...
      {/* 4. Count — rovnaký ako Dashboard */}
      {filtered.length > 0 && (
        <div style={css.count}>{filtered.length} targetov</div>
      )}

      {/* 5. Karty — rovnaký list layout ako Dashboard */}
      {filtered.length === 0 ? (
        <div style={css.empty}>
          {targets.length === 0
